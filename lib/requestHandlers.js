...
function send(response, query) {
	if(query.key) {
		fs.readFile("./cfg/key", function(err, key){
			if(err)
				throw err;

			if(key.toString() === query.key.toString()) {
				fs.readFile("./src/send.html", function(err, html){
					if(err)
						throw err;
...
function save(response, query) {
	if(query.msg)
		msgStorage.saveMessage(query.msg, function(result, id) {
			...
			var html = "<!DOCTYPE html><html><head><title>Sending message</title></head><body>___server_message___<script>history.replaceState({}, null, \"/send\");</script></body></html>"
